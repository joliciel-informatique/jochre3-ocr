...
trait Lexicon {
  ...
  /** If [[hasFrequency]], this will return an actual corpus-based frequency, 0 for an unknown word,
    * and -1 for an impossible word.
    *
    * If not, this will return 1 for a known word, 0 for an unknown word, and -1 for an impossible
    * word.
    *
    * The "word" here is actually a string without whitespace, but could contain punctuation as well
    * as one or more words (e.g. separated by a dash).
    */
