...
case class TextLineExtractor(
  ...
  def annotateOneFile(
    ...
    alto.pages.head.textLinesWithRectangles.zipWithIndex.foreach { case ((textLine, rectangle), i) =>
...
object TextLineExtractor {
  private class TextLineExtractorCLI(arguments: Seq[String]) extends ScallopConf(arguments) {
