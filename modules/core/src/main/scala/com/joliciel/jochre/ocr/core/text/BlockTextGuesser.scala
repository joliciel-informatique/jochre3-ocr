...
object BlockTextGuesserService {
  val live: ZLayer[ImageToAltoConverter, Nothing, TextGuesserService] =
    ZLayer.fromFunction(BlockTextGuesserServiceImpl(_))
}

private[text] case class BlockTextGuesserServiceImpl(
  ...
  override def guess(
    ...
    for {
      ...
      segments <- ZIO.attempt {
        val imageSegmentExtractor =
          ImageSegmentExtractor(mat, page.blocks, debugLocation)
        imageSegmentExtractor.segments
      }
      pageWithContent <- ZIO
        .foreach(segments.zipWithIndex) {
          case (TextSegment(block, subImage), i) =>
            ...
            debugLocation.foreach { outputLocation =>
              saveImage(
                fromBufferedImage(subImage),
                outputLocation.resolve(f"_textblock$i.png")
              )
            }

            imageToAltoConverter
              .analyze(subImage)
              .map { altoXml =>
                val jochreSubImage = Page.fromXML(altoXml)
                val translatedSubImage =
                  jochreSubImage.translate(block.left, block.top)
        ...
        .mapAttempt { blocks =>
          val sortedBlocks = BlockSorter
            .sort(blocks.flatten, leftToRight)
