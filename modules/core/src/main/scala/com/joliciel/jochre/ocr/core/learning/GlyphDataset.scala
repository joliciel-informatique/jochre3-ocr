  ...
  with FileUtils
{
  ...
  private def getImages(manager: NDManager): Seq[Array[Float]] = images.zip(pages).flatMap{ case (imageFile, page) =>
    ...
    allGlyphs.map{ glyph =>
      ...
      val crop = new Crop(glyph.rectangle.left - leftMargin, glyph.rectangle.top - topMargin, height, height)
  ...
  override def get(manager: NDManager, index: Long): dataset.Record = {
    ...
    val label: NDList = new NDList(manager.create(getClassNumber(index).toFloat))
