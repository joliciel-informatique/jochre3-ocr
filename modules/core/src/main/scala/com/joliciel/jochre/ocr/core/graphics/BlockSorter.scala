...
/** Block sorting algorithm.
  ...
  * }}}
  *
  * We call a "vertical break", a block which overlaps two or more blocks horizontally. Above, 3 and
  * 7 are vertical breaks. We order the blocks such that: if a vertical break separates two blocks
  * vertically and overlaps them horizontally, we order top-down: e.g. 2 comes before 4, 6 comes
  * before 8. If on the other hand, there is no such vertical break then: If there is horizontal
  * overlap, we use top-down ordering: e.g. 3 comes before 4. If there is no horizontal overlap, we
  * use right-to-left ordering: e.g. 3 comes before 6, 4 comes before 7, 4 comes before 5, 4 comes
  * before 6, 5 comes before 6
  */
case class BlockSorter(blocks: Seq[WithRectangle], leftToRight: Boolean)
    extends Ordering[WithRectangle] {
    ...
    if (verticalBreak.isDefined) {
      ...
      if (a.horizontalOverlap(b) > 0) {
        ...
        if (log.isTraceEnabled) {
          ...
          log.trace(
            f"No horizontal overlap: horizontal compare. ${a.rectangle.coordinates} $comparator ${b.rectangle.coordinates}"
          )
        }
        result
      }
