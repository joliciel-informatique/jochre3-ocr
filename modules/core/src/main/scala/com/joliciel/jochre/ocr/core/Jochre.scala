...
trait AbstractJochre extends Jochre with ImageUtils with FileUtils with XmlImplicits {
  ...
  override def processPdf(
    ...
    val converter = PDFToImageConverter(
  ...
  override def processDirectory(
    ...
    ZIO
      .foreach(files.zipWithIndex) { case (inputFile, i) =>
  ...
  override def processImageFile(
    ...
    this.processMat(
      mat,
      fileName.getOrElse(imageFile.toFile.getName),
      outputFormats,
      outputDir,
      debugDir,
      testRectangle
  ...
  private def processImageInternal(
      bufferedImage: BufferedImage,
      fileName: String,
  ...
  private def processMatInternal(
      mat: Mat,
      fileName: String,
      physicalPageNumber: Int = 1,
      debugDir: Option[Path] = None,
    ...
    for {
      ...
      transformedAndData <- ZIO.attempt {
        transforms.foldLeft(mat -> Seq.empty[Any]) { case ((mat, outputData), transformer) =>
      ...
      pageWithContent <- textGuesser.guess(
        segmented,
        transformed,
        fileName,
        debugLocation
      ...
      alto <- ZIO.attempt {
        val allConfidences = pageWithContent.blocks
          .flatMap {
        ...
        debugLocation.foreach { debugLocation =>
          ...
          saveImage(
            transformedLabelled,
            debugLocation.resolve("_transformed_seg.png")
          )
