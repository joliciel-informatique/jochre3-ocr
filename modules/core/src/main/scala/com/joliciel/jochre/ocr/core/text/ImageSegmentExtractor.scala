...
private[text] case class ImageSegmentExtractor(
    image: Mat,
    blocks: Seq[Block],
  ...
  val segments: Seq[ImageSegment] = {
    ...
    blocks
      .collect { case illustration: Illustration =>
        illustration
      }
    ...
    outputLocation.foreach(outputLocation =>
      ImageIO.write(
        ...
        "png",
        outputLocation.resolve("_no_illustraions.png").toFile
      )
    )
    ...
    blocks.flatMap {
      case textBlock: TextContainer =>
        ...
        val left = if (rectangle.left < 0) { 0 }
        else { rectangle.left }
        val width =
          if (left + rectangle.width > withoutIllustrations.getWidth) {
            withoutIllustrations.getWidth - left
          } else { rectangle.width }
        val top = if (rectangle.top < 0) { 0 }
        else { rectangle.top }
        val height =
          if (top + rectangle.height > withoutIllustrations.getHeight) {
            withoutIllustrations.getHeight - top
          } else { rectangle.height }
        val subImage =
          withoutIllustrations.getSubimage(left, top, width, height)
