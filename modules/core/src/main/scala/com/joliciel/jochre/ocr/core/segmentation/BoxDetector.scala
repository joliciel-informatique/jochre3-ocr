...
class BoxDetector(thresh: Int = 240, override val outputLocation: Option[OutputLocation] = None) extends ImageLabelDetector[Rectangle] {
  def detect(image: Mat, label: String): Seq[Rectangle] = {
    outputLocation.foreach(outputLocation => imwrite(outputLocation.resolve(f"_${label}_box0_image.png").toString, image))

    val gray = new Mat()
    threshold(image, gray, thresh, 255, THRESH_BINARY)
    outputLocation.foreach(outputLocation => imwrite(outputLocation.resolve(f"_${label}_box1_thresh.png").toString, gray))

    val canny = new Mat()
    Canny(gray, canny, 175.0, 200.0, 3, true)
    outputLocation.foreach(outputLocation => imwrite(outputLocation.resolve(f"_${label}_box4_canny.png").toString, canny))
