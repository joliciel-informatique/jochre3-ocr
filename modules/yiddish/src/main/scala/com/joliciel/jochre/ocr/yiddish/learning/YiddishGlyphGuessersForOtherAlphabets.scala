...
object YiddishGlyphGuessersForOtherAlphabets {
  private val configs = ConfigFactory
    .load()
    .getConfigList("jochre.ocr.yiddish.glyph-guesser-for-other-alphabets")
    .asScala

  private val yiddishConfig =
    ConfigFactory.load().getConfig("jochre.ocr.yiddish.glyph-guesser")
  ...
  private val guessers = configs.map { config =>
    ...
    val modelType =
      ModelType.withName(config.getString("model-type")).glyphTrainerModelType
    val glyphGuesser = GlyphGuesser(modelDir, modelName, modelType)
    GlyphGuesserForAnotherAlphabet(language, regex, glyphGuesser)
  }.toSeq

  private val glyphGuessersForOtherAlphabets = GlyphGuessersForOtherAlphabets(
    guessers
  )

  val live: ZLayer[Any, Throwable, GlyphGuessersForOtherAlphabets] =
    ZLayer.succeed(glyphGuessersForOtherAlphabets)
}
