...
object YivoLexiconService {
  val live: ZLayer[YiddishConfig, Nothing, YivoLexiconService] =
    ZLayer.fromFunction {
...
private class YivoLexiconImpl(entries: Set[String]) extends TextFileLexicon(entries) with YivoLexicon {
  ...
  private val punctuationSplitter =
    raw"""(?U)((?<=\p{Punct}+)|(?=\p{Punct}+))""".r
  ...
  private def splitWordOnPunctuation(word: String): Seq[Part] = {
    if (punctuationAndNotRegex.findFirstIn(word).isDefined) {
      ...
      val splitTriplets = (punctuationSplits :+ "" :+ "")
      ...
      val correctedSplits = punctuationSplits.zipWithIndex.flatMap { case (split, i) =>
        if (abbreviationIndexes.contains(i)) {
          Some(
            Part.Abbreviation(
              punctuationSplits(i - 1) ++ split ++ punctuationSplits(i + 1)
            )
          )
      ...
      ArraySeq.unsafeWrapArray(correctedSplits)
    } else {
  ...
  private val impossibleWordRegex =
    raw"""(?U)(\w*[ןםךץף]\w+)|(\w*[LC]\w*[א-ת]\w*)|(\w*[א-ת]\w*[LC]\w*)|(\w*\D\w*\d\w*\D\w*)""".r
